        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <Clock className="h-5 w-5 text-muted-foreground inline" />
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="bg-card rounded-xl p-4 shadow-sm border border-border">
            <div className="flex items-center gap-2 mb-2">
              <div className={`w-2.5 h-2.5 rounded-full ${
                settings?.crowdLevel === "high" ? "bg-red-500" :
                settings?.crowdLevel === "medium" ? "bg-yellow-500" :
                "bg-green-500"
              }`} />
              <span className="text-xs text-muted-foreground">Crowd Level</span>
            </div>
            <p className={`text-2xl font-bold ${
              settings?.crowdLevel === "high" ? "text-red-500" :
              settings?.crowdLevel === "medium" ? "text-yellow-500" :
              "text-green-500"
            }`}>
              {settings?.crowdLevel === "high" ? "High" :
               settings?.crowdLevel === "medium" ? "Medium" :
               "Low"}
            </p>
          </div>
          <div className="bg-card rounded-xl p-4 shadow-sm border border-border">
        ...
        <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide snap-x snap-mandatory">
          {popularItems.map((item) => (
            <button
              key={item.id}
              onClick={() => navigate("/menu")}
              className="snap-start flex-shrink-0 w-36 bg-card rounded-xl shadow-sm border border-border overflow-hidden text-left active:scale-95 transition-transform"
            >
