  ...
  create: publicQuery
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      for (const item of cart) {
        ...
        totalAmount += Number(item.foodItem.price) * item.cartItem.quantity;
      }
      ...
      const qrPayload = { orderId, studentId, tokenNumber, timestamp: Date.now() };
      ...
      return { orderId, tokenNumber, totalAmount, qrCode: qrCodeDataUrl };
    }),
  ...
  cancel: publicQuery
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (order.orderStatus !== "pending" && order.orderStatus !== "preparing") {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Order can only be cancelled if it is pending or preparing",
        });
      }
  ...
  updateStatus: adminQuery
    ...
    .mutation(async ({ input }) => {
      ...
      if (input.status === "collected") {
        updateData.collectionStatus = 1;
        updateData.collectionDate = new Date().toISOString();
        // Mark payment as paid for cash-on-delivery orders
  ...
  scanQr: adminQuery
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const { signature, ...qrPayload } = data;
